...
function make_slides(f) {
  ...
  function mapIntention(sentence){
    if(sentence == "How likely is it that Brock's actions are intentional?"){
      return "intention2"
    } else if(sentence ==  "How likely is it that Brock knows what the boxes do?"){
      return "knowledge"
    } else if(sentence == "Was Brock aiming to activate the boxes?"){
      return "goalBox"
...
slides.intentionH = slide({
  ...
  present : randomSort([
    {sentence: "How likely is it that Brock's actions are intentional?"}, //intention
    {sentence: "How likely is it that Brock knows what the boxes do?"}, //knowledge
    {sentence: "Was Brock aiming to open the door?"}, //goal
    {sentence: "Was Brock aiming to clean the room?"}, //goalClean
    {sentence: "Was Brock aiming to activate the boxes?"} //goalBox
  ]),
...
slides.attention2 =  slide({ //check this!!!
  ...
  button : function(){
    ...
    if($('input[name=attention2]:checked').length == 0) {
      $(".err").show();
    }  else {
    ...
    append(exp.data,
      {
        ...
        "attention2" : $('input[name="attention2"]:checked').val()
      }
      )
...
function sendDataToServer(data) {
	$.ajax({
    url: 'https://script.google.com/macros/s/AKfycby1yblJmb669lAOUO8xy4VTpkxpWE5XCHoV_SlH0rHGuJHpiSCiXr_iUpYRIJYWol6I/exec',
		type: 'post',
    data: data,
    complete: function(){
      window.location.replace("https://app.prolific.co/submissions/complete?cc=87381A90") // redirect to prolific
    } 
	});
}
